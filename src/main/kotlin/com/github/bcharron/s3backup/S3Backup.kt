package com.github.bcharron.s3backup

import aws.sdk.kotlin.services.s3.S3Client
import aws.sdk.kotlin.services.s3.model.ListObjectsRequest
import java.io.File

class S3Backup(
    val client: S3Client,
) {
    companion object {
        fun ensureTrailingSlash(s: String) = s.trimEnd('/').plus('/')
    }

    suspend fun sync(
        localPath: String,
        bucketName: String,
        remotePath: String,
    ) {
        val bucketFiles = listBucketFiles(bucketName, remotePath)
        val localFiles = listLocalFiles(localPath)

        val differences = findDifferences(localFiles, bucketFiles)

        differences.forEach { println("Different: ${it.relativePath}") }
    }

    suspend fun findDifferences(
        localFiles: List<AFile>,
        bucketFiles: List<AFile>,
    ): List<AFile> {
        val bucketFileSet = bucketFiles.associate { it.relativePath to it }

        val differences =
            localFiles.filter { it ->
                val bucketEntry = bucketFileSet[it.relativePath]

                bucketEntry == null || !it.isSimilarSize(bucketEntry)
            }

        return differences
    }

    suspend fun listLocalFiles(path: String): List<AFile> {
        val directory = File(path)

        if (!directory.exists()) {
            throw IllegalArgumentException("Directory does not exist: $path")
        }

        if (!directory.isDirectory) {
            throw IllegalArgumentException("Provided path is not a directory: $path")
        }

        return directory
            .walkTopDown()
            .filter { it.isFile }
            .map { AFile.fromPath(it.path, path, it.length()) }
            .toList()
    }

    suspend fun listBucketFiles(
        bucketName: String,
        prefix: String,
    ): List<AFile> {
        val cleanPrefix = ensureTrailingSlash(prefix)

        val req =
            ListObjectsRequest {
                bucket = bucketName
                this.prefix = cleanPrefix
            }

        client.use { s3 ->
            val response = s3.listObjects(req)

            return response.contents
                .orEmpty()
                .filter { it.key?.startsWith(cleanPrefix) == true && it.size != null }
                .map { AFile.fromPath(it.key!!, cleanPrefix, it.size!!) }
        }
    }
}
